...
default persistent.clickstart = float(1.2)
...
init python:
    ...
    class Par(renpy.Displayable):
        ...
        def event(self, ev, x, y, st):
            ...
            onclick = ev.type == pygame.MOUSEBUTTONDOWN
            offclick = ev.type == pygame.MOUSEBUTTONUP
            # mousefocus = pygame.mouse.get_focused()
            if hover:
                self.x, self.y = x, y
            if onclick:
                persistent.clickstart = time.time()
            elif offclick:
                persistent.clickend = time.time() - persistent.clickstart
                if persistent.clickend < 0.7:
                    renpy.jump(persistent.markshortclick)
            return
